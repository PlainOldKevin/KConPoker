...
export class HandRank {
    ...
    static evaluate(game, cardgroup) {
        ...
        let flushSuit = 0;
        some(Object.keys(countBySuits), (suit) => {
        ...
        if (flushSuit > 0) {
            if (straightCardsCount >= 5) {
                const straightFlushCards = filter(cardgroup, (card) => {
                ...
                if (straightFlushCards.length >= 5) {
                    ...
                    const aceCards = filter(cardgroup, (card) => {
        ...
        if (quadRanks.length === 1) {
            const quadCards = filter(cardgroup, (card) => card.getRank() === quadRanks[0]);
            const cards = reject(cardgroup, (card) => card.getRank() === quadRanks[0]);
        ...
        if (tripRanks.length === 1 && pairRanks.length >= 1) {
            const tripCards = filter(cardgroup, (card) => {
                return card.getRank() === tripRanks[0];
            });
            const pairCards = filter(cardgroup, (card) => {
        ...
        // Flush
        if (flushSuit > 0) {
            const flushCards = filter(cardgroup, (card) => {
        ...
        // Straight
        if (straightCardsCount === 5) {
            const straightCards = uniqWith(filter(cardgroup, (card) => {
        ...
        if (tripRanks.length === 1) {
            ...
            const cards = reject(cardgroup, (card) => {
                return card.getRank() === tripRanks[0];
            });
        ...
        if (pairRanks.length >= 2) {
            const pairedHigherCards = filter(cardgroup, (card) => {
                return card.getRank() === pairRanks[0];
            });
            const pairedLowerCards = filter(cardgroup, (card) => {
                return card.getRank() === pairRanks[1];
            });
            const unpairedCards = reject(reject(cardgroup, (card) => card.getRank() === pairRanks[0]), (card) => card.getRank() === pairRanks[1]);
        ...
        if (pairRanks.length === 1) {
            const pairedCards = filter(cardgroup, (card) => {
                return card.getRank() === pairRanks[0];
            });
            const unpairedCards = reject(cardgroup, (card) => {
                return card.getRank() === pairRanks[0];
            });
